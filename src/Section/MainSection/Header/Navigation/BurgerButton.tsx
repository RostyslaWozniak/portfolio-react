...
const BurgerButton = () => {
    ...
    return ( 
        <button
            ...
            <span 
                className="absolute w-full top-0 left-0 h-[5px] bg-primary rounded duration-300"
                style={{
                    transform: isNavActive ? 'translateY(13px) rotate(-45deg)' : 'translateY(0) rotate(0)'
                }}
            ></span>
            <span 
                className="absolute w-[70%] top-[50%] right-0 h-[5px] bg-primary rounded translate-y-[-50%] group-hover:w-[100%] duration-300"
                style={{
                    display: isNavActive ? 'none' : 'block'
                }}
            ></span>
            <span 
                className="absolute w-full bottom-0 left-0 h-[5px] bg-primary rounded duration-300"
                style={{
                    transform: isNavActive ? 'translateY(-13px) rotate(45deg)' : 'translateY(0) rotate(0)'
                }}
            ></span>
